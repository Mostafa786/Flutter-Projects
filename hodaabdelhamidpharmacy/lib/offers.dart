...
class _OffersPageState extends State<OffersPage> {
  ...
  List Offers_Product = [];
  offers_Function_detect(){
    for(int i = 0;i<Product.length;i++){
      if(Product[i]["Special Offer"] != ""){
        Offers_Product.addAll([Product[i]]);
      }
    }
  }
  @override
  ...
  Widget build(BuildContext context) {
      return Scaffold(bottomNavigationBar: BottomNavigationBar(iconSize:28,type: BottomNavigationBarType.fixed,currentIndex:3,onTap: (val){setState(() {
        if(val == 0){
          ...
          Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context)=>OffersPage()));
        }
        else{
          ...
          body: Container(padding: EdgeInsets.all(10),
              child: Offers_Product.length > 0?ListView(physics: BouncingScrollPhysics(),
                children: [
                  Container(margin: EdgeInsets.only(left: 10),child: Text("Offers (${Offers_Product.length})",maxLines: 1,overflow: TextOverflow.ellipsis,style: TextStyle(fontSize: 30,fontWeight: FontWeight.w900,fontFamily: "Times New Roman"),textAlign: TextAlign.left,)),
                  Container(child:GridView.builder(itemCount: Offers_Product.length,physics: NeverScrollableScrollPhysics(),shrinkWrap: true,gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2,mainAxisExtent: 450),itemBuilder: (context, index) {
                    return Container(child: Card(child: Column(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Container(height: 250,
                          child: Stack(
                            children: [
                              Image.asset(Offers_Product[index]["Image"],height: 300,),
                              if(Offers_Product[index]["Best Seller"])Positioned(right: 0,top: 0,child:Container(margin: EdgeInsets.only(right: 5,top: 5),padding: EdgeInsets.all(7),decoration: BoxDecoration(color: Colors.amber,borderRadius: BorderRadius.circular(30)),child: Text("Best Seller",style: TextStyle(color: Colors.white),),)),
                              if(Offers_Product[index]["Special Offer"] != "")Positioned(right: 0,bottom: 0,child:Container(padding: EdgeInsets.only(top:5,left: 5),width: 110,decoration: BoxDecoration(borderRadius: BorderRadius.only(topLeft: Radius.circular(10)),color: Colors.redAccent,backgroundBlendMode: BlendMode.hardLight),child: Text(Offers_Product[index]["Special Offer"],maxLines: 2,overflow: TextOverflow.ellipsis),)),
                              Positioned(left: 0,top: 0,child:Container(margin: EdgeInsets.only(left: 10,top: 10),child:InkWell(onTap: (){setState(() {
                                Offers_Product[index]["Favorite"] = !Offers_Product[index]["Favorite"];
                              });},child:Offers_Product[index]["Favorite"]?Icon(Icons.favorite_rounded,color: Colors.red,):Icon(Icons.favorite_border_rounded)))),
                            ],),
                        ),
                        Container(margin: EdgeInsets.only(left: 5),alignment: Alignment.centerLeft,child: Text("${Offers_Product[index]["Drug Name"]}",style: TextStyle(fontWeight: FontWeight.w700,fontSize: 25),textAlign: TextAlign.left,overflow: TextOverflow.ellipsis,maxLines: 2,)),
                        Container(margin: EdgeInsets.only(left: 5),alignment: Alignment.centerLeft,child: Text("${Offers_Product[index]["Drug Category"]}",style: TextStyle(color: Colors.grey),textAlign: TextAlign.left,overflow: TextOverflow.ellipsis,maxLines: 1,),),
                        Container(margin: EdgeInsets.only(left: 5),alignment: Alignment.centerLeft,child: Text("${Offers_Product[index]["Drug Price"]} Pound",style: TextStyle(color: Colors.indigo,fontSize: 25,fontWeight: FontWeight.w900),textAlign: TextAlign.left,overflow: TextOverflow.ellipsis,maxLines: 1,),),
                ...
                Container(margin: EdgeInsets.only(left: 10),alignment: Alignment.centerLeft,child: Text("Offers",maxLines: 1,overflow: TextOverflow.ellipsis,style: TextStyle(fontSize: 30,fontWeight: FontWeight.w900,fontFamily: "Times New Roman"),textAlign: TextAlign.left,)),
                Container(child: Image.asset(fit: BoxFit.cover,"images/noOffers.png")),
                Container(margin: EdgeInsets.only(left: 10),child: Text("No offers yet",overflow: TextOverflow.ellipsis,maxLines: 2,style: TextStyle(fontSize: 25,fontWeight: FontWeight.w700),)),
